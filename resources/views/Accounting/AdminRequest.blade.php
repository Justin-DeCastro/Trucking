...
<div class="dt-buttons btn-group d-flex justify-content-end gap-2 ">
                                     ...
                                     <table class="jobOffersTable">
                                        ...
                                        <tbody>
                                            ...
                                            <tr id="row-{{ $budget->id }}">
                                                <td>{{ \Carbon\Carbon::parse($budget->date)->format('F d, Y') }}</td>
                                                ...
                                                <td>
                                                    <form action="{{ route('budgets.approve', $budget->id) }}" method="POST" style="display:inline;" class="approve-form">
                                                        @csrf
                                                        <button type="submit" class="btn btn-success approve-btn {{ $budget->status == 'Approved' ? 'disabled' : '' }}"
                                                                {{ $budget->status == 'Approved' ? 'disabled' : '' }}>
                                                            Approve
                                                        </button>
                                                    </form>
                                                    <form action="{{ route('budgets.deny', $budget->id) }}" method="POST" style="display:inline;" class="deny-form">
                                                        @csrf
                                                        <button type="submit" class="btn btn-danger deny-btn {{ $budget->status == 'Denied' ? 'disabled' : '' }}"
