                        ...
                        <div class="table-responsive--sm table-responsive">
                            <table id="data-table" class="table table--light style--two display nowrap">
                                ...
                                <tbody>
                                    ...
                                    <tr class="clickable-row" data-id="{{ $singil->id }}" data-issuer="{{ $singil->issuer }}" data-borrower="{{ $singil->borrower }}" data-principal="{{ $singil->principal }}" data-mode="{{ $singil->mode_of_payment }}" data-pay-now="{{ $singil->pay_now_date }}" data-pay-later="{{ $singil->pay_later_date }}">
                                        <td>{{ \Carbon\Carbon::parse($singil->date_borrowed)->format('d-M-y') }}</td>
                                        <td>{{ $singil->issuer }}</td>
                                        <td>{{ $singil->borrower }}</td>
                                        <td>{{ $singil->principal }}</td>
                                        <td>{{ $singil->mode_of_payment }}</td>
                                        <td>{{ $singil->pay_now_date ? \Carbon\Carbon::parse($singil->pay_now_date)->format('d-M-y') : '-' }}</td>
                                        <td class="text-start">{{ $singil->pay_later_date ? \Carbon\Carbon::parse($singil->pay_later_date)->format('d-M-y') : '-' }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
    ...
    <div class="modal fade" id="detailsModal" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
        ...
        <div class="modal fade" id="manageSubcontractor" tabindex="-1" aria-labelledby="manageSubcontractorLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form action="{{ route('receivables.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <button type="button" id="payNowBtn" class="btn btn-primary w-100" onclick="selectPayNow()">Pay Now</button>
                                <input type="date" id="pay_now_date" name="pay_now_date" class="form-control mt-2" disabled required>
                            </div>

                            <div class="col-md-6">
                                <button type="button" id="payLaterBtn" class="btn btn-secondary w-100" onclick="selectPayLater()">Pay Later</button>
                                <input type="date" id="pay_later_date" name="pay_later_date" class="form-control mt-2" disabled required readonly>
                            </div>
                        </div>
                    </div>
                ...
                <div id="payLaterCalendar" style="display: none; position: fixed; right: 190px; top: 100px; z-index: 1000; background: white; border: 1px solid #ccc; padding: 10px; width: 300px;">
...
<script>
    ...
    document.querySelectorAll('.clickable-row').forEach(row => {
        row.addEventListener('click', function () {
            ...
            const payNow = this.getAttribute('data-pay-now') ? new Date(this.getAttribute('data-pay-now')).toLocaleDateString() : '-';
            const payLater = this.getAttribute('data-pay-later') ? new Date(this.getAttribute('data-pay-later')).toLocaleDateString() : '-';
...
<script>
    ...
    $(document).ready(function() {
        ...
        document.getElementById('printBtn').addEventListener('click', function() {
            ...
            var printableHtml = `<table><thead><tr>${headers.map(header => `<th>${header}</th>`).join('')}</tr></thead><tbody>${data.map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
