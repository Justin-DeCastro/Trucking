...
<body>
    ...
    <tbody>
        @foreach($paymentproof as $paymentproofs)
        <tr>
            <td>{{ $paymentproofs->amount}}</td>
            
           
    @if($paymentproofs->proof_of_payment)
        <a href="{{ asset($paymentproofs->proof_of_payment) }}" target="_blank">View File</a>
    @else
        No File
    @endif
</td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="manageSubcontractorLabel">Upload Proof of Payment</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('proofpayment.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="modal-body">
        <div class="row mb-3">
            ...
            <div class="col-md-6">
                <label for="proof-of-payment" class="form-label">Proof of Payment</label>
                <input type="file" id="proof-of-payment" name="proof_of_payment" class="form-control" required>
            </div>
        </div>
    </div>
